...
execute if entity @e[type=item,tag=dbp_dispensered,tag=in_air] as @e[type=item,tag=dbp_dispensered,tag=in_air] at @s unless entity @p[scores={disable_es=1..}] if block ~ ~-1 ~ minecraft:end_stone run tag @s add end_stone_plantable
